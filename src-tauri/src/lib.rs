    ...
    std::thread::spawn(move || {
        let mut run = || -> Result<(), Box<dyn std::error::Error>> {
            ...
            let mut vad = SileroVad::new(
                SileroVadConfig {
                    model: asset(&window, "silero_vad.onnx")
                        .to_str()
                        .unwrap()
                        .to_string(),
            ...
            let mut rec = SenseVoiceRecognizer::new(SenseVoiceConfig {
                model: asset(&window, "model.int8.onnx")
                    .to_str()
                    .unwrap()
                    .to_string(),
            ...
            std::thread::spawn(move || {
                ...
                for (id, text) in rx {
                    let live: Config = fs::read_to_string(&config_path)
                        ...
                        .unwrap_or_else(|| cfg_t.clone());
                    let trans = translate(&http, &live, &text);
            ...
            loop {
                ...
                while !vad.is_empty() {
                    let seg = vad.front();
                    if (seg.samples.len() as f32 / 16000.0) < 0.3 {
                        vad.pop();
                        continue;
                    }
